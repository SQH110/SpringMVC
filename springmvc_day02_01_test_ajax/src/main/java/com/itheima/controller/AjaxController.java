...
//    }
    ...
    @RequestMapping("/ajaxController")
    //使用@RequestBody注解，可以将请求体内容封装到指定参数中（这里的message接收的在Ajax中是请求正文）
    public String ajaxController(@RequestBody String message){//想要接收数据，需要一个变量保存数据
...
//    @ResponseBody
    public @ResponseBody String ajaxReturnString(){//加注解响应正文（写什么回什么），使得回调的时候不是一个页面
    ...
    @ResponseBody
    //基于jackon技术，使用@ResponseBody注解可以将返回的POJO对象转成json格式数据（前后端分离）
